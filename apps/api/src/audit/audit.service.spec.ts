...
describe('AuditService', () => {
  it('filters and paginates operation logs with actor information', async () => {
    const prisma = {
      ...
      $transaction: vi.fn(async () => [[{ id: 'log-id', actor: { displayName: '管理员' } }], 1]),
    };
    const service = new AuditService(prisma as never);
    await expect(service.list({ page: 2, pageSize: 20, actorUserId: '00000000-0000-4000-8000-000000000001', action: 'UPDATE', occurredFrom: '2026-09-01', occurredTo: '2026-09-07' })).resolves.toMatchObject({ total: 1 });
