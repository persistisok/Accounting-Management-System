...
export function BankingPage() {
  ...
  const create = useMutation({
    mutationFn: (body: Record<string, string>) => api.post('/banking/transactions', { ...body, settlementApplicable: true, sourceType: 'MANUAL' }),
    onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ['banking'] }); setTransactionModal(false); },
  });
  ...
  const columns: TableColumn<BankTransaction>[] = [
    ...
    { key: 'allocation', label: '已关联', render: (row) => row.allocations.length ? <span className="allocation-list">{row.allocations.filter((item) => item.status !== 'REVERSED').map((item) => <small key={item.id}>{statusLabels[item.category]} · {item.project?.projectCode ?? item.expertProfile?.person.name ?? '会费'} · {formatMoney(item.allocatedAmount)}</small>)}</span> : <span className="muted">尚未关联</span> },
    { key: 'status', label: '匹配状态', render: (row) => <StatusChip value={row.matchStatus} /> },
    { key: 'action', label: '', render: (row) => <button className="table-action" onClick={(event) => { event.stopPropagation(); setSelected(row); setCategory(row.direction === 'IN' ? 'SUPPORT_RECEIPT' : 'EXECUTION_PAYMENT'); }} disabled={row.matchStatus === 'MATCHED' || row.matchStatus === 'EXCLUDED'}><GitBranchPlus size={15} />分配</button> },
  ];
  ...
  return <div className="page-enter">
    ...
    <Modal open={transactionModal} onClose={() => setTransactionModal(false)} title="录入银行流水" description="保存后可继续把金额分配到项目或会费。" size="large">
      <form className="form-grid" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); create.mutate(formObject(event.currentTarget)); }}>
        <Field label="本方银行账户" span={2}><Select name="bankAccountId" required><option value="">请选择</option>{accounts.data?.map((item) => <option key={item.id} value={item.id}>{item.accountName} · {item.accountNumberMasked}</option>)}</Select></Field>
        <Field label="银行流水号"><Input name="transactionNo" /></Field><Field label="交易时间"><Input name="transactionAt" type="datetime-local" required /></Field>
        <Field label="对方账户名称"><Input name="counterpartyName" required /></Field><Field label="收支方向"><Select name="direction"><option value="IN">收入</option><option value="OUT">支出</option></Select></Field>
        <Field label="金额"><MoneyInput name="amount" min="0.01" required /></Field><Field label="摘要 / 性质"><Input name="nature" required /></Field>
        {create.error && <p className="form-error span-2">{create.error.message}</p>}
        <div className="span-2"><FormActions pending={create.isPending} onCancel={() => setTransactionModal(false)} submitLabel="保存流水" /></div>
      </form>
    </Modal>
