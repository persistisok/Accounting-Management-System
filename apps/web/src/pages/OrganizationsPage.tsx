...
export function OrganizationsPage({ roleType }: { roleType: 'SUPPORTER' | 'EXECUTOR' }) {
  ...
  return <div className="page-enter">
    <PageHeader eyebrow={`基础资料 / ${isSupporter ? '支持方' : '执行方'}`} title={`${isSupporter ? '支持方' : '执行方'}库`} description={isSupporter ? '签署支持协议时自动查重并复用已有机构。' : '执行方入库并启用后，可直接用于登记执行协议。'} action={canEdit ? <button className="button primary" onClick={() => setModal(true)}><Plus size={17} />新增{isSupporter ? '支持方' : '执行方'}</button> : undefined} />
