...
import { DateInput, Input, Select } from '../components/FormControls';
...
import type { ListResponse, Project, ProjectFilterOptions } from '../lib/types';
...
export function ProjectOverviewPage() {
  ...
  const projects = useQuery({
    queryKey: ['project-overview', q, platform, nature, projectType, publishedFrom, publishedTo, page],
    queryFn: () => api.get<ListResponse<Project>>(`/projects${queryString({
      q, platform, nature, projectType, publishedFrom, publishedTo, page, pageSize: DEFAULT_PAGE_SIZE,
    })}`),
  });
  ...
  const columns: TableColumn<Project>[] = [
    ...
    { key: 'pm', label: 'PM', render: (row) => <span className="overview-pm"><i>{(row.pm?.displayName ?? row.pmName).slice(0, 1)}</i><strong>{row.pm?.displayName ?? row.pmName}</strong></span> },
  ...
  const activeFilterCount = [q, platform, nature, projectType, publishedFrom, publishedTo].filter(Boolean).length;
  return <div className="page-enter project-overview-page">
    <PageHeader eyebrow="项目管理 / 横向总览" title="项目全览" description="按统一口径横向核对立项、支持协议与实际回款。" action={user?.role === 'SYSTEM_ADMIN' ? <LedgerExportButton dataset="projects" fileName="项目全览" filters={{ q, platform, nature, projectType, publishedFrom, publishedTo }} /> : undefined} />
